  ...
  const validateForm = () => {
    ...
    if (images.length === 0) {
      ...
      images.forEach((image, index) => {
        ...
        if (image.size > 5 * 1024 * 1024) {  // Max size: 5MB
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      try {
        ...
        const productResponse = await axios.post('http://localhost:8080/api/v1/products/add', formData, {
          headers: {
    ...
    <div className="container">
      ...
      <form id="productForm" onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <input type="number" id="inventory" name="inventory" value={formData.inventory} onChange={handleChange} min="0" required />
          {errors.inventory && <p style={{ color: 'red' }}>{errors.inventory}</p>}
        </div>
        <div>
        ...
        <div>
          ...
          <div className="image-upload" onClick={() => document.getElementById('images').click()}>
            ...
            <input type="file" id="images" name="images" accept="image/*" multiple style={{ display: 'none' }} onChange={handleImageChange} />
          </div>
